...
=== Fetch snapshot
Fetches info about stored snapshot by ID. Possible snapshot statuses are `PENDING`, `SUCCESS` and `FAILED`. Single path
argument is the snapshot ID.
...
`SUCCESS` and `FAILED`. Available query parameters are `chainId` and `status` and both are optional. Multiple values
can be specified for `status` query parameter by separating them with commas. Requesting user is specified implicitly
